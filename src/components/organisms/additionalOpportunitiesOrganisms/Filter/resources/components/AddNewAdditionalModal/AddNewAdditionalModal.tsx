...
const AddNewAdditionalModal: FC<IAddNewAdditionalModalModalProps> = ({ onClose }) => {
    ...
    const { mutate, isPending } = useMutation({
        ...
        onSuccess: (response: TCriticalAny) => {
            toast.success(response.data.message ?? 'تعریف فرصت اضافه برای شرکت ها با موفقیت انجام شد')
        ...
        onError: (error: TCriticalAny) => {
            toast.error(error.data.message || 'تعریف فرصت اضافه برای شرکت ها با مشکل مواجه شد')
        }
    })
        ...
        <form
            ...
            onSubmit={handleSubmit((value) =>
                mutate({
                    ...
                    dateTimeLimit: convertDateMicroseconds(value.dateTimeLimit),
                    exceptionRuleType: 1
                })
            )}
        >
            ...
            <Controller
                ...
                control={control}
                render={({ field }) => (
                    <SInputField label='مهلت ارسال' errors={errors} name={field.name}>
                        <SDatePicker {...field} placeholder='مهلت ارسال را وارد کنید' />
                    </SInputField>
                )}
            />

...
import { postAddReportMutationFn } from '@core/services/api/report/post-add-report'
...
import { addReportTypeSchema, type IAddReportTypeModalProps, type TAddReportTypeForm } from './resources'

const AddReportTypeModal: FC<IAddReportTypeModalProps> = ({ onClose }) => {
    ...
    const {
        ...
        setValue,
        watch
    } = useForm<TAddReportTypeForm>({
        resolver: yupResolver(addReportTypeSchema)
    })

    const documentFormatIdWatch = watch('documentFormatId')
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: postAddReportMutationFn,
            ...
            <Controller
                ...
                render={({ field }) => (
                    ...
                    >
                        <SDropZone
                            accept={`.${
                                documentFormatIdWatch
                                    ? allDocumentFormats?.data
                                          .find((item) => item.id === +documentFormatIdWatch)
                                          ?.format.toLowerCase()
                                    : ''
                            }`}
                            {...field}
...
export default AddReportTypeModal

...
import { postUpdateUserMutationFn } from '@core/services/api/user/post-update-user'
...
const EditUserModal: FC<IEditUserModalProps> = ({ onClose, data }) => {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: postUpdateUserMutationFn,
    ...
    return (
        ...
        >
            ...
            <Controller
                ...
                render={({ field }) => (
                    <SInputField label='شماره تلفن' errors={errors} name={field.name}>
                        <SNumberInput maxLength={11} {...field} placeholder='شماره تلفن را انتخاب کنید' />
                    </SInputField>
                )}
            />
            <Controller
            ...
            <Controller
                name='role'
                control={control}
                defaultValue={data?.companyPositionId.toString() ?? ''}
                render={({ field }) => (
